...
pub struct LeaderSwapTable {
    ...
    good_nodes: Vec<(AuthorityIndex, Stake)>,
    ...
    bad_nodes: HashMap<AuthorityIndex, Stake>,
}
...
impl Debug for LeaderSwapTable {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_str(&format!(
            ...
            self.good_nodes.iter().map(|a| *a.1).sum::<Stake>(),
            self.bad_nodes
                .iter()
                .map(|a| *a.0)
                .collect::<Vec<AuthorityIndex>>(),
            self.bad_nodes.iter().map(|a| *a.1).sum::<Stake>(),
        ))
    }
}
...
impl LeaderSwapTable {
    ...
    pub fn new(
        ...
        let bad_nodes = Self::retrieve_first_nodes(
            ...
            .collect::<HashMap<AuthorityIndex, Stake>>();
    ...
    /// the final schedule.
    pub fn swap(&self, leader: &AuthorityIndex, leader_round: RoundNumber) -> Option<Authority> {
...
impl LeaderSchedule {
    ...
    pub fn leader(&self, round: RoundNumber) -> AuthorityIndex {
        assert_eq!(
            round % 2,
            0,
