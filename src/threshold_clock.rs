...
use crate::types::{BlockReference, Committee, MetaStatementBlock, RoundNumber};
...
pub fn threshold_clock_valid(block: &MetaStatementBlock) -> bool {
    ...
    let round_number = block.get_reference().1;
    ...
    round_number == 0 || committee.is_quorum(committee.get_total_stake(&authorities_with_includes))
}

// Take a committee and read the next round number from self, and for each round number
// With any included blocks, check if we have included blocks from enough authorities to make
// a quorum. Return the highest round number for which we have a quorum.
pub fn get_highest_threshold_clock_round_number(
    committee: &Committee,
    blocks_processed_by_round: &HashMap<RoundNumber, Vec<BlockReference>>,
    min_round: RoundNumber,
) -> Option<RoundNumber> {
    // Here we store the highest round number for which we have a quorum
    let mut result = None;
    let mut authorities_with_includes = HashSet::new();

    // Get the next round number
    let mut round_number = min_round;
    loop {
        if let Some(blocks) = blocks_processed_by_round.get(&round_number) {
...
mod tests {
    ...
    fn test_threshold_clock_valid() {
        ...
        // make another block from authority 1 at round number 0 including block0
        let block1 =
            MetaStatementBlock::new_for_testing(&auth1, 0).extend_with(block0.into_include());
        assert!(!threshold_clock_valid(&block1)); // fails because block0 is at round number 0

        // Make a block at round number 1 including block1
        let block2 =
            MetaStatementBlock::new_for_testing(&auth2, 1).extend_with(block1.into_include());
        assert!(!threshold_clock_valid(&block2)); // fails because not enough round number 0 blocks included
        ...
        assert!(threshold_clock_valid(&block3)); // succeeds because there is a quorum of round number 0 blocks
    }

    fn make_test_reference(auth: &Authority, round: RoundNumber) -> BlockReference {
        (auth.clone(), round, 0)
    }
    ...
    /// Check that the highest threshold clock valid round number is 2
    #[test]
    fn test_get_highest_threshold_clock_valid_round_number() {
        let committee = Committee::new(0, vec![1, 1, 1, 1]);
        let auth0 = committee.get_rich_authority(0);
        let auth1 = committee.get_rich_authority(1);
        let auth2 = committee.get_rich_authority(2);
        ...
        let result =
            get_highest_threshold_clock_round_number(&committee, &blocks_processed_by_round, 0);
        ...
        let result =
            get_highest_threshold_clock_round_number(&committee, &blocks_processed_by_round, 1);
